$(function(){
	...
	function onloadData(){
		$.ajax({
			type:"Post",
			url:"NoticeServlet?method=queryNoticeInfoByUserID",
			dataType:"Json",
			success:function(data){
				...
				if(data != null){
					for(i; i < length; i++){
						...
						+'<span class="s-theme">【'+data[i].noticeTheme+'】</span>'
						+'<span class="s-type"><time class="timeago" datetime="'+data[i].noticeTime+'">0天前</time></span>'
						+'</div>'
						+'<div class="notice-content clearfix">'
						+'<p class="p-content">'+data[i].noticeContent+'</p>'
						+'</div></div><!--END 循环部位-->'
