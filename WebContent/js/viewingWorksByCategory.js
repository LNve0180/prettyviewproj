 $(function(){
	 onloadUserInfo();
	 var allWorksNum=0;
	 var selectedWorksCategory=localStorage.getItem("clickedWorkCategory");
		$(".seeWorksDisplayTop span").html("作品展示区("+selectedWorksCategory+")");	
		
	//初次加载数据
		for(var i=0;i<2;++i){
			$.ajax({
				type:"Post",
				url:"WorksServlet?method=queryWorksInfoByWorksCategory",
				dataType:"Json",
				data : {
					"worksCategory" : selectedWorksCategory
				},
	...
	$("#seeCategoryWorksAddA").click(function(){
		$.ajax({
			type:"Post",
			url:"WorksServlet?method=queryWorksInfoByWorksCategory",
			dataType:"Json",
