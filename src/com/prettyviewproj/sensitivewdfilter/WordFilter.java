package com.prettyviewproj.sensitivewdfilter;
...
import com.prettyviewproj.sensitivewdfilter.util.BCConvert;
...
public class WordFilter {
	 ...
	 * 
	 * @param src
	 * @return sensitiveAllWords
	 */
	public static final String doSearch(final String src) {
		if (set != null && nodes != null) {
			char[] chs = src.toCharArray();
			int length = chs.length;
			int currc; // 当前检查的字符
			int cpcurrc; // 当前检查字符的备份
			int k;
			char[] sensitiveWords = new char[length];//创建敏感词字符数组
			String sensitiveAllWords="";
			...
			for (int i = 0; i < length; i++) {
				...
				if (couldMark) {
					for (k = 0; k <= markNum; k++) {
						//System.out.println(chs[k+i]);
						sensitiveWords[k] = chs[k + i];
						chs[k + i] = SIGN;
					}
					i = i + markNum;
					//System.out.println(new String(sensitiveWords));
					sensitiveAllWords += (new String(sensitiveWords)).trim()+"，";//将敏感词转换为数组形式
					
				}
			}
			if(sensitiveAllWords !="") {
				sensitiveAllWords = sensitiveAllWords.substring(0,sensitiveAllWords.length() - 1);//去除最后一位逗号
				//System.out.println(sensitiveAllWords);
